...
TEST_F(TestCertiSearch, TestCertiSearchInvalid) {
	...
	auto results = searcher->search(employee_management, Inform{ "certi", "ABC" });
	EXPECT_EQ(0, results->size());
}

TEST_F(TestCertiSearch, TestCertiSearchNotExist) {
	auto searcher = make_unique<CertiSearch>();
	auto results = searcher->search(employee_management, Inform{ "certi", "ADV" });
	EXPECT_EQ(0, results->size());
}

TEST_F(TestCertiSearch, TestCertiSearchExist) {
	auto searcher = make_unique<CertiSearch>();
	auto results = searcher->search(employee_management, Inform{ "certi", "EX" });
